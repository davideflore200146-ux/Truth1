    ...
    <View style={{ flex: 1 }}>
      ...
      <View style={{ paddingHorizontal: 18, paddingTop: 6 }}>
        ...
        {!loading && !error && items.length === 0 && (
          <Text style={styles.empty}>Nessuna analisi ancora. Torna alla Home per iniziare.</Text>
        )}
        {!loading &&
          !error &&
          items.map((h) => {
            ...
            return (
              <Card key={h.id} onPress={() => onOpen(h)} style={styles.row}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                  ...
                  {h.currentPrice != null && <Text style={styles.price}>€{h.currentPrice}</Text>}
                </View>
              </Card>
            );
          })}
...
const styles = StyleSheet.create({
  ...
  name: { fontWeight: '600', fontSize: 13.5, color: COLORS.textPrimary, flex: 1 },
