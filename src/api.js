...
export const analyzeProduct = (query) =>
  request('/api/analyze', { method: 'POST', body: JSON.stringify({ query }) });

export const getWishlist = () => request('/api/wishlist');

export const addToWishlist = (product) =>
  request('/api/wishlist', { method: 'POST', body: JSON.stringify({ product }) });

export const removeFromWishlist = (id) => request(`/api/wishlist/${id}`, { method: 'DELETE' });

export const getHistory = () => request('/api/history');

export const sendChatMessage = (analysisId, question) =>
  request('/api/chat', { method: 'POST', body: JSON.stringify({ analysisId, question }) });
